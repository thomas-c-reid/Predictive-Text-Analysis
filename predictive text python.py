...
# TODO work for 2/3 levels simultaneously
...
# start with one level analysis - then two - then three - set variable at start and increament when loop finished
...
def Train(file, level):
    ...
    #             if temp_phrase not in unique_words_2:
    #                 unique_words_2.append(temp_phrase)
    ...
    for line in file:
        ...
        for word in words:
            for unique_word in unique_words:
                # print("searched word: " + unique_word + " | Checked word: " + word + " | line: " + line )
                Next_words = get_Next_Words(words, unique_word, level)
                temp_JSON = build_final_JSON(Next_words, word)
    ...
    #         # Next_words = Find(file, word)
    #         print("First -------------")
    #         Next_words = get_Next_Words(line_of_words, word.lower(), level)
    #         print(Next_words)
    #         print()
    #         # print(Next_words)
    #         temp_JSON = build_final_JSON(Next_words, word)
...
def get_Next_Words(words, searched_word, level):
        ...
        if (word_index + 1) < len(words):
            if word_index < level:
                ...
                if temp_phrase == searched_word:
                    next_indexes.append(word_index)
            elif word_index > level:
    ...
    # hello, my, name, is, thomas, hello, my, how, are, you
    ...
    for index in next_indexes:
        if index < len(words):
            temp_word = words[index + 1]
            follow_up_words.append(temp_word)
    ...
    #         if temp_phrase == searched_word:
    #             next_indexes.append(last_index)
    #         temp_phrase = ""
    #         word_index += 1
    # for indexes in next_indexes:
    #     if indexes < len(words):
    #         temp_word = words[indexes]
    #         follow_up_words.append(temp_word)
...
def print_data(trained_data, length, level):
    final_phrase = ""
    changing_phrase = ""
...
with open("textFiles/testText.txt", 'r') as f:
    level = 5
    trained_data = Train(f, level)
    for data in trained_data:
        phrase = print_data(trained_data, 10, level)
        print(phrase)
